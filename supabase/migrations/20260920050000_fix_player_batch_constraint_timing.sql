-- Fix transactional player writes when the API transaction has constraint checks set to IMMEDIATE.
-- The player row and its player_batches mapping must be created/updated in the same
-- transaction before the invariant constraint trigger runs.
...
declare
  ...
  v_player_id uuid;
begin
  set constraints trg_enforce_player_batch_invariant deferred;

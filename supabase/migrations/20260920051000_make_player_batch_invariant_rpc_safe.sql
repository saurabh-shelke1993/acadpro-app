...
--
-- The constraint trigger remains the database-level guard for ordinary direct writes.
-- Transactional player RPCs temporarily mark their own transaction so the trigger
-- does not reject the intentional intermediate state between the players insert/update
-- and player_batches insert/update. Each RPC performs an explicit invariant check
-- before returning.
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(p_academy_id::text || ':' || v_phone, 0)
  );
  ...
  values (
    p_academy_id, v_parent_id, trim(p_full_name), p_dob,
    nullif(trim(p_gender), ''), p_joining_date, 'active', true,
    p_center_id, p_batch_id, v_phone
  )
  ...
  perform public.enforce_player_batch_invariant(
    (select p from public.players p where p.id = v_player_id),
    null
  );
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(v_player_academy_id::text || ':' || v_phone, 0)
  );
  ...
  perform public.enforce_player_batch_invariant(v_player_row, null);
...
begin
  ...
  loop
    ...
    if v_player_name is null
      ...
      raise exception 'Import validation failed for source row %.',
        coalesce(v_row->>'sourceRowNumber', '?');
    end if;

    if v_gender is not null and lower(v_gender) not in ('male', 'female') then
      raise exception 'Import validation failed for source row %: invalid gender.',
        coalesce(v_row->>'sourceRowNumber', '?');
    end if;

    if v_parent_email is not null
       and v_parent_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then
      raise exception 'Import validation failed for source row %: invalid parent email.',
        coalesce(v_row->>'sourceRowNumber', '?');
    end if;
    ...
    exception when invalid_text_representation then
      raise exception 'Import validation failed for source row %: invalid center or batch reference.',
        coalesce(v_row->>'sourceRowNumber', '?');
    end;
    ...
    if not exists (
      ...
      raise exception 'Center "%" is no longer valid for the selected academy (source row %).',
        v_center_name, coalesce(v_row->>'sourceRowNumber', '?');
    end if;
    ...
    if not exists (
      ...
      raise exception 'Batch "%" is no longer valid for center "%" (source row %).',
        v_batch_name, v_center_name, coalesce(v_row->>'sourceRowNumber', '?');
    end if;

    perform pg_advisory_xact_lock(
      hashtextextended(p_academy_id::text || ':' || v_parent_phone, 0)
    );
    ...
    if v_parent_count > 1 then
      raise exception 'Multiple active parents already use phone "%" (source row %).',
        v_parent_phone, coalesce(v_row->>'sourceRowNumber', '?');
    elsif v_parent_count = 1 then
      ...
      if v_parent_email is not null
        ...
        raise exception 'Parent email does not match the existing parent with phone "%" (source row %).',
          v_parent_phone, coalesce(v_row->>'sourceRowNumber', '?');
      end if;
    else
    ...
    if exists (
      ...
      raise exception 'Player "%" already exists with this parent phone (source row %).',
        v_player_name, coalesce(v_row->>'sourceRowNumber', '?');
    end if;
    ...
    values (
      p_academy_id,v_parent_id,v_player_name,v_date_of_birth,v_gender,
      v_joining_date,'active',true,v_center_id,v_batch_id,v_parent_phone
    )
    returning id into v_player_id;

    insert into public.player_batches (player_id,batch_id)
    values (v_player_id,v_batch_id);
...
revoke execute on function public.update_player_transactional(uuid,uuid,uuid,text,text,text,text,date,text,date,uuid,uuid) from public;

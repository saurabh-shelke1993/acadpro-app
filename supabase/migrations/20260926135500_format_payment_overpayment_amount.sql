-- AcadPro: format overpayment amount without unnecessary trailing zeros.

-- AcadPro: fix overpayment validation message formatting.
-- PostgreSQL RAISE uses '%' as the format placeholder; '%s' leaves a literal 's'.
...
begin
  ...
  if v_new_paid > v_due.total_amount then
    raise exception
      ...
      trim(to_char(v_due.total_amount - coalesce(v_due.paid_amount, 0), 'FM999999999999990.##'));
  end if;
